...
<template>
  <div :class="{
    ...
    <div class="row">
      <div class="col-7 p0 title">
        <slot name="title"></slot>
      </div>
      <div class="col-5 p0 date">
        <slot name="date"></slot>
      </div>
    </div>
    <div class="subtitle">
...
.card {
  position: relative;
  background: rgb(23 22 32) !important;
  border-radius: 5px;
  color: #888;
  user-select: none;
  margin-bottom: 15px;
...
.card:hover {
  background: rgb(28, 27, 40) !important;
  transition: .2s;
}
...
.card .bottom .action span::before {
  content: "";
  width: 0%;
  height: 100%;
  position: absolute;
...
.card .bottom .action span:hover img {
  opacity: .7;
  transition: .1s;
}
...
.card .bottom .action span:hover .text::before,
.card .bottom .action span:hover::before {
  width: 100%;
}
